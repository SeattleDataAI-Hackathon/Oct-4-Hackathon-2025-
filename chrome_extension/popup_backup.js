...
class ExtractorPopup {
    ...
    async loadSettings() {
        const result = await chrome.storage.sync.get(['autoExtract']);
    ...
    setupEventListeners() {
        // Auto-extract toggle
        document.getElementById('auto-extract-toggle').addEventListener('click', (e) => {
            const toggle = e.currentTarget;
            const isActive = toggle.classList.contains('active');
            
            if (isActive) {
                toggle.classList.remove('active');
                chrome.storage.sync.set({ autoExtract: false });
            } else {
                toggle.classList.add('active');
                chrome.storage.sync.set({ autoExtract: true });
            }
        });
                ...
                if (this.currentSustainabilityReport.error) {
                    ...
                    reportContent.innerHTML = `
                        ...
                        <div style="margin-top: 15px; text-align: center;">
                            ${fileName ? `
                                <button onclick="window.open('http://127.0.0.1:5000/reports/${fileName}', '_blank')" 
                                        style="padding: 8px 16px; background: #007bff; color: white; border: none; border-radius: 4px; cursor: pointer; margin-right: 5px;">
                                    📄 Open Full Report
                                </button>
                            ` : ''}
                            <button onclick="navigator.clipboard.writeText('${content.replace(/'/g, "\\'")}').then(() => alert('Report copied to clipboard!'))" 
