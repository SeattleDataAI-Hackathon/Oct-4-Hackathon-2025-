...
def get_mistral_api_key():
    try:
        print(Fore.GREEN + "##[debug]Getting Mistral API key...")
        ...
        if not api_key:
            raise Exception("MISTRAL_API_KEY not found in environment variables. Please set your Mistral API key.")

        print(Fore.GREEN + "##[debug]Mistral API key retrieved successfully!")
        return api_key
    except Exception as e:
        print(Fore.RED + f"[##error] Error getting Mistral API key: {e}")
        return None

def get_embeddings(api_key):
    try:
        print(Fore.GREEN + "##[debug]Initializing Mistral Embeddings...")
        ...
        print(Fore.GREEN + "##[debug]Mistral Embeddings initialized successfully!")
        return embeddings
    except Exception as e:
...
def store_embeddings_in_faiss(api_key, chunks, faiss_index_path="medical_faiss_index_mistral", source_file_path="./datasets/pregHelp.txt"):
    try:
...
def store_embeddings_in_faiss2(api_key, chunks, faiss_index_path="medical_faiss_index_mistral"):
    try:
        print(Fore.GREEN + "##[debug]Storing embeddings in FAISS...")
...
def create_faiss_rag(api_key, faiss_store, n=5):
    try:
        # Initialize the LLM model
        print(Fore.GREEN + "##[debug]Initializing Mistral LLM model...")
        ...
        print(Fore.GREEN + "##[debug]Mistral LLM model initialized successfully.")
